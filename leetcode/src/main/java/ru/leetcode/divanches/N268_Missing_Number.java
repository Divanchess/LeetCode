...
public class N268_Missing_Number {
    ...
    public static int missingNumber(int[] nums) {
        // Решение основано на формуле, что сумма n первых натуральных чисел равна n(n+1)/2
        int n = nums.length;
    ...
    public static int missingNumber2(int[] nums) {
        Set<Integer> seen = new HashSet<>(IntStream.rangeClosed(0, nums.length).boxed().toList());
        for (int i : nums) {
            seen.remove(i);
        }
        if (seen.size() > 1) {
            throw new IllegalArgumentException("nums missing more than one number");
        }
